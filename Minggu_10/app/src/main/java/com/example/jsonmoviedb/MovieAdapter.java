...
public class MovieAdapter extends RecyclerView.Adapter<MovieAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(MovieAdapter.ViewHolder holder, int position){
        ...
        float newValues = (float) Rating;
        holder.ratingBar.setNumStars(5);
        holder.ratingBar.setStepSize((float) 0.5);
        holder.ratingBar.setRating(newValues / 2);

        Glide.with(mContext).load(ApiEndpoint.URLIMAGE + data.getPosterPath()).apply( new RequestOptions()
                .placeholder(R.drawable.baseline_image_24)
                .transform(new RoundedCorners(16))).into(holder.imgPhoto);
